...
const writeModelOrType = ({ fileWriter: { writer, writeImport, writeImportSet, writeJSDoc, writeHeading, }, dmmf, getSingleFileContent = false, }, model) => {
    ...
    if (useMultipleFiles && !getSingleFileContent) {
        ...
        if (createRelationValuesTypes && model.hasRelationFields) {
            ...
            writeImportSet(new Set(model.filterdRelationFields
                .map((field) => {
    ...
    if (model.writePartialTypes) {
        writer.blankLine();
        writeHeading(`${model.formattedNames.upperCaseSpace} PARTIAL SCHEMA`, 'SLIM');
        writer
            .blankLine()
    ...
    if (model.writeOptionalDefaultValuesTypes) {
        writer.blankLine();
        writeHeading(`${model.formattedNames.upperCaseSpace} OPTIONAL DEFAULTS SCHEMA`, 'SLIM');
        writer
            .blankLine()
    ...
    if (model.writeRelationValueTypes) {
        writer.blankLine();
        writeHeading(`${model.formattedNames.upperCaseSpace} RELATION SCHEMA`, 'SLIM');
        writer
            .blankLine()
    ...
    if (model.writeOptionalDefaultsRelationValueTypes) {
        writer.blankLine();
        writeHeading(`${model.formattedNames.upperCaseSpace} OPTIONAL DEFAULTS RELATION SCHEMA`, 'SLIM');
        writer.blankLine();
        if (model.hasOptionalJsonFields) {
            writer
    ...
    if (model.writePartialRelationValueTypes) {
        writer.blankLine();
        writeHeading(`${model.formattedNames.upperCaseSpace} PARTIAL RELATION SCHEMA`, 'SLIM');
        writer
            .blankLine()
        ...
        if (model.hasOptionalJsonFields) {
            ...
            writer.write(`export type ${model.name}PartialWithRelations = z.infer<typeof ${model.name}PartialSchema> & `);
        }
